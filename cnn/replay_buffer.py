...
class MiniBuffer:
    ...
    def reset_buffer(self, obs_dim, action_dim):
        ...
        self.buffer['map'] = [[[] for k in range (self.episode_limit)]for i in range(self.batch_size)]

        self.max_episode_len = 0

    def store_transition(self, num_episode, episode_step, state, v, a, a_logprob, r, done,agent_id,map):
...
class BigBuffer:
    ...
    def get_training_data(self, device):
        ...
        for key in buffer:
            if key == 'v':
                ...
                buffer[key] = torch.tensor(buffer[key]).to(device)
            else:
    ...
    def concat_mini_buffer(self, mini_buffer):
        for key in self.buffer.buffer:
            if key == 'map':
                self.buffer.buffer[key] = self.buffer.buffer[key] + mini_buffer.buffer[key]
            else:
